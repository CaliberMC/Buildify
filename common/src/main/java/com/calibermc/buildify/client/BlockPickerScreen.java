...
public class BlockPickerScreen extends Screen implements MenuAccess<BlockPickerMenu> {
    ...
    private void renderFloatingItem(@NotNull GuiGraphics guiGraphics, ItemStack pStack, int pX, int pY) {
        ...
        guiGraphics.renderItemDecorations(this.font, pStack, pX, pY - (this.draggingItem.isEmpty() ? 0 : 8));
    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
        if (!super.mouseClicked(pMouseX, pMouseY, pButton)) {
            boolean flag = this.getMinecraft().options.keyPickItem.matchesMouse(pButton);
                ...
                if (l != -1) {
                    if (this.getMinecraft().options.touchscreen().get()) {
                        ...
                        if (this.menu.getCarried().isEmpty()) {
                            if (this.getMinecraft().options.keyPickItem.matchesMouse(pButton)) {
            ...
            if (hasShiftDown()) {
                if (!lastQuickMoved.isEmpty()) {
                    for (Slot slot2 : this.menu.slots) {
                        if (this.getMinecraft().player != null && slot2 != null && slot2.mayPickup(this.getMinecraft().player) && slot2.hasItem() && slot2.container == slot.container && AbstractContainerMenu.canItemQuickReplace(slot2, lastQuickMoved, true)) {
    ...
    public boolean keyPressed(int pKeyCode, int pScanCode, int pModifiers) {
        ...
        if (super.keyPressed(pKeyCode, pScanCode, pModifiers)) {
            return true;
        } else if (this.getMinecraft().options.keyInventory.matchesMouse(mouseKey.getValue())) {
            this.onClose();
            return true;
        } else {
            ...
            if (this.hoveredSlot != null) {
                ...
                if (this.menu.getCarried().isEmpty()) {
                    for (int i = 0; i < 9; ++i) {
                        if (this.getMinecraft().options.keyHotbarSlots[i].matchesMouse(InputConstants.getKey(pKeyCode, pScanCode).getValue())) {
                ...
                if (this.hoveredSlot.hasItem()) {
                    if (this.getMinecraft().options.keyPickItem.matchesMouse(mouseKey.getValue())) {
                        this.slotClicked(this.hoveredSlot, this.hoveredSlot.index, 0, ClickType.CLONE);
                        handled = true;
                    } else if (this.getMinecraft().options.keyDrop.matchesMouse(mouseKey.getValue())) {
                        this.slotClicked(this.hoveredSlot, this.hoveredSlot.index, hasControlDown() ? 1 : 0, ClickType.THROW);
                        handled = true;
                    }
                } else {
                    // if slot has no item, don't drop
                    if (this.getMinecraft().options.keyDrop.matchesMouse(mouseKey.getValue())) {
                        handled = true;
                    }
                }
            }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (this.menu.getCarried().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
            ...
            MutableComponent mutablecomponent = (Component.literal("")).append(stack.getHoverName()).withStyle(stack.getRarity().color);
            if (stack.hasCustomHoverName()) {
                mutablecomponent.withStyle(ChatFormatting.ITALIC);
            }
            int w = width - this.font.width(mutablecomponent) / 2;
            int h = this.height - 22 - 10;
            guiGraphics.drawString(this.font, mutablecomponent, w, h, 16777215);
        }
